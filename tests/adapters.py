...
def run_transformer_lm(
    ...
    # Remap reference weight keys to match our module's parameter names:
    # - attn.{q,k,v}_proj.weight -> attn.{q,k,v}_proj.W  (custom Linear uses .W)
    # - attn.output_proj.weight -> attn.o_proj.W
    # - token_embeddings.weight, ln*.weight, ffn.w*.weight, lm_head.weight stay the same
    remapped = {}
    for k, v in weights.items():
        k = k.replace(".attn.q_proj.weight", ".attn.q_proj.W")
        k = k.replace(".attn.k_proj.weight", ".attn.k_proj.W")
        k = k.replace(".attn.v_proj.weight", ".attn.v_proj.W")
        k = k.replace(".attn.output_proj.weight", ".attn.o_proj.W")
        remapped[k] = v
